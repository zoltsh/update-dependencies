...
export interface ExactUpdateExecutionInput {
...
export interface ExactUpdateExecutionDependencies {
    readonly exactUpdate?: typeof runExactUpdate;
    readonly verify?: typeof verifyLockedOffline;
}

export async function prepareExactUpdateArtifact(
    input: ExactUpdateExecutionInput,
    dependencies: ExactUpdateExecutionDependencies = {},
    ...
    try {
        ...
        const result = await (dependencies.exactUpdate ?? runExactUpdate)(
            input.binary,
            zoltRoot,
            input.environment,
            {
...
function requireAuthoritativeTarget(
    ...
    if (!target.authoritativeTarget || !isCanonicalDigestIdentifier(target.targetId, 'zt1_')) {
        throw actionError(
            'ZOLT-EXECUTE-001',
            'Exact update execution requires an authoritative schema-v2 Zolt target.',
        );
    }
    const zoltManifestPath = canonicalZoltManifestPath(
        target.zoltManifestPath,
        'planned Zolt manifest path',
    );
    const zoltLockfilePath = canonicalZoltRootLockPath(
        target.zoltLockfilePath,
        'planned Zolt lockfile path',
    );
    if (
        target.zoltRoot !== selection.relativeRoot
        ...
        throw actionError(
            'ZOLT-EXECUTE-001',
            'The exact update target does not belong to the selected Zolt mutation root.',
        );
    }
}

function validateExactResult(result: ExactUpdateResult, target: PlannedUpdate): void {
    ...
    if (!sameStrings(result.fanOut, target.fanOut)) mismatches.push('fanOut');
    ...
    if (mismatches.length !== 0) {
        throw actionError(
            'ZOLT-EXECUTE-002',
            `Zolt exact-update result did not match the selected target: ${mismatches.join(', ')}.`,
        );
    }
}
...
function validateChanges(
    ...
    const reported = result.changedFiles.map((path) =>
        joinRelativeRoot(target.zoltRoot, path, 'Zolt-reported changed file'));
    ...
    if (!sameStrings(reported, expectedOrder)) {
        throw actionError(
            'ZOLT-EXECUTE-004',
            'Zolt-reported changed files did not exactly match the independently observed manifest/root-lock changes.',
        );
    }
...
): Promise<readonly UpdateArtifactFile[]> {
...
function requireSameChanges(before: RepositoryChangeSet, after: RepositoryChangeSet): void {
    const fileChanges = (changes: RepositoryChangeSet): object => ({
        added: changes.added,
    ...
    if (JSON.stringify(fileChanges(before)) !== JSON.stringify(fileChanges(after))) {
        throw actionError(
            'ZOLT-EXECUTE-006',
            'Locked offline verification changed the prepared update artifact.',
        );
    }
}

function requireSameFiles(
    before: readonly UpdateArtifactFile[],
    after: readonly UpdateArtifactFile[],
): void {
    if (
