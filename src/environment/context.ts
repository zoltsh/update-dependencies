...
export async function readExecutionContext(
    ...
    readEvent: (path: string) => Promise<string> = readEventFile,
    ...
    if (!ALLOWED_EVENTS.has(eventName)) {
        throw actionError(
            'ZOLT-EVENT-002',
            `Event ${eventName} is not supported. Run on schedule, workflow_dispatch, or a default-branch push.`,
        );
    }
    const eventPath = required(environment.GITHUB_EVENT_PATH, 'GITHUB_EVENT_PATH');
    const raw = await readEvent(eventPath);
    if (Buffer.byteLength(raw) > MAX_EVENT_BYTES) throw actionError('ZOLT-EVENT-003', 'The GitHub event payload is too large.');
...
function required(value: string | undefined, name: string): string {
    if (value === undefined || value.trim() === '') throw actionError('ZOLT-EVENT-009', `${name} is not set.`);
    return value;
}
