...
import type { RepositoryChangeSet } from '../types.js';
...
export async function inspectRepositoryTree(
    ...
    async function visit(directory: string, prefix: string): Promise<void> {
        ...
        for (const child of children) {
            const path = canonicalRelativeFile(
            ...
            if (info.isDirectory()) {
                ...
                if (!expectedDirectories.has(path)) unexpectedDirectories.push(path);
            ...
            if (expected === undefined) {
                added.push(path);
                if ((info.mode & 0o111) !== 0) modeChanged.push(path);
                continue;
            }
            seenFiles.add(path);
            const executable = (info.mode & 0o111) !== 0;
            if (executable !== (expected.mode === '100755')) modeChanged.push(path);
...
export async function verifyRepositoryTree(
    root: string,
    entries: readonly RepositoryTreeEntry[],
): Promise<void> {
    ...
    if (changes.unexpectedDirectories.length !== 0) {
        throw actionError(
            'ZOLT-REPOSITORY-009',
            `Immutable repository view contains an unexpected directory: ${changes.unexpectedDirectories[0]}.`,
        );
    }
    if (changes.missingDirectories.length !== 0) {
        throw actionError(
            'ZOLT-REPOSITORY-009',
            `Immutable repository directory changed during analysis: ${changes.missingDirectories[0]}.`,
        );
    }
    if (changes.added.length !== 0) {
        throw actionError(
            'ZOLT-REPOSITORY-009',
            `Immutable repository view contains an unexpected file: ${changes.added[0]}.`,
        );
    }
    if (changes.deleted.length !== 0) {
        throw actionError(
            'ZOLT-REPOSITORY-009',
            `Immutable repository entry changed during analysis: ${changes.deleted[0]}.`,
        );
    }
    if (changes.modeChanged.length !== 0) {
        throw actionError(
            'ZOLT-REPOSITORY-009',
            `Immutable repository mode changed during analysis: ${changes.modeChanged[0]}.`,
        );
    }
    if (changes.modified.length !== 0) {
        throw actionError(
            'ZOLT-REPOSITORY-009',
            `Immutable repository bytes changed during analysis: ${changes.modified[0]}.`,
        );
    }
}

function directoryPaths(entries: readonly RepositoryTreeEntry[]): ReadonlySet<string> {
...
async function gitBlobId(path: string, size: number, objectLength: number): Promise<string> {
