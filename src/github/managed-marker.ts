...
export type ManagedMarkerParseResult =
    | { readonly kind: 'invalid'; readonly reason: string }
    | { readonly kind: 'none' }
    | { readonly kind: 'valid'; readonly marker: ManagedPullRequestMarker };

export function renderManagedMarker(marker: ManagedPullRequestMarker): string {
    const validated = validateMarker(marker);
    const payload = Buffer.from(JSON.stringify(validated), 'utf8').toString('base64url');
    return `${PREFIX}${payload} -->`;
}

export function parseManagedMarker(body: string): ManagedMarkerParseResult {
    ...
    try {
        ...
        return Object.freeze({ kind: 'valid', marker: decodeMarkerObject(JSON.parse(source) as unknown) });
    } catch (error) {
        return invalid(error instanceof Error ? error.message : 'The managed marker could not be decoded.');
    }
}

function decodeMarkerObject(value: unknown): ManagedPullRequestMarker {
...
function validateMarker(marker: ManagedPullRequestMarker): ManagedPullRequestMarker {
    ...
    if (
        !insideRoot(zoltRoot, manifestPath)
...
function schemaVersion(value: unknown): 1 {
    if (value !== 1) throw markerError('The managed marker schema is unsupported.');
    return 1;
}
...
function invalid(reason: string): ManagedMarkerParseResult {
    return Object.freeze({ kind: 'invalid', reason });
}

function markerError(message: string): ReturnType<typeof actionError> {
