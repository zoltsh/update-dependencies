...
import type { ManagedPublicationResult } from './publication.js';
...
export interface SummaryContext {
    ...
    readonly publication?: ManagedPublicationResult;
...
export function renderSummary(context: SummaryContext): string {
    ...
    const deferred = deferredTargets.map((target) =>
    ...
    const publicationBlockers = context.publication?.reconciliation.blocked.map(({ existing, reason }) =>
        `| #${existing.number.toString()} | \`${escapeCell(existing.branch)}\` | ${escapeCell(reason)} |`,
    ) ?? [];
...
${table(
        '| Pull request | Branch | Reason |\n| :---: | :--- | :--- |',
        publicationBlockers,
        '_No managed pull requests were blocked._',
    )}
...
function publicationStatus(publication: ManagedPublicationResult | undefined): string {
    if (publication === undefined) return '> Planning preview: no branches or pull requests were written.';
...
function publicationFooter(publication: ManagedPublicationResult | undefined): string {
