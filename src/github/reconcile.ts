...
export interface MatchedManagedPullRequest {
    readonly desired: DesiredManagedPullRequest;
    readonly existing: ExistingPullRequest;
    readonly marker: ManagedPullRequestMarker;
}

export interface BlockedManagedPullRequest {
    readonly existing: ExistingPullRequest;
    readonly reason: string;
}

export interface ManagedPullRequestReconciliation {
    readonly blocked: readonly BlockedManagedPullRequest[];
    readonly close: readonly ExistingPullRequest[];
    readonly create: readonly DesiredManagedPullRequest[];
    readonly deferred: readonly DesiredManagedPullRequest[];
    readonly ignored: readonly ExistingPullRequest[];
    readonly refresh: readonly MatchedManagedPullRequest[];
    readonly unchanged: readonly MatchedManagedPullRequest[];
}

export interface RetainedManagedTarget {
...
interface DecodedExisting {
    readonly existing: ExistingPullRequest;
    readonly marker: ManagedPullRequestMarker;
}

export function reconcileManagedPullRequests(
    ...
    const refresh: MatchedManagedPullRequest[] = [];
    const unchanged: MatchedManagedPullRequest[] = [];
    const close: ExistingPullRequest[] = [];
    const claimedDesired = new Set<string>();

    for (const [managedId, entries] of [...groups].sort(([left], [right]) => left.localeCompare(right))) {
        if (desiredById.has(managedId)) claimedDesired.add(managedId);
        if (entries.length !== 1) {
...
function desiredIndex(
    ...
    for (const entry of desired) {
        ...
        if (result.has(entry.target.managedId)) {
            throw actionError(
                'ZOLT-RECONCILE-001',
                `Duplicate desired managed identity ${entry.target.managedId}.`,
            );
        }
...
function requireDesiredTarget(entry: DesiredManagedPullRequest, branchGeneration: string): void {
    ...
    if (
        ...
        throw actionError(
            'ZOLT-RECONCILE-001',
            'Managed pull request reconciliation requires authoritative schema-v2 targets.',
        );
    }
    ...
    const zoltManifestPath = canonicalZoltManifestPath(
        target.zoltManifestPath,
        'reconciliation Zolt manifest path',
    );
    const zoltLockfilePath = canonicalZoltRootLockPath(
        target.zoltLockfilePath,
        'reconciliation Zolt lockfile path',
    );
    if (
        joinRelativeRoot(root, zoltManifestPath, 'reconciliation manifest path') !== manifest
...
function retainedIndex(
    retained: readonly RetainedManagedTarget[],
    desired: ReadonlyMap<string, DesiredManagedPullRequest>,
...
function requireRepositoryIdentity(repositoryId: string, defaultBranch: string, baseSha: string): void {
...
function groupByManagedId(
    entries: readonly DecodedExisting[],
): ReadonlyMap<string, readonly DecodedExisting[]> {
    const grouped = new Map<string, DecodedExisting[]>();
    for (const entry of entries) {
...
function markerMismatch(
    marker: ManagedPullRequestMarker,
    desired: DesiredManagedPullRequest,
): string | null {
...
function comparePullRequests(left: ExistingPullRequest, right: ExistingPullRequest): number {
    return left.number - right.number;
}

function compareBlocked(left: BlockedManagedPullRequest, right: BlockedManagedPullRequest): number {
    return comparePullRequests(left.existing, right.existing);
}

function compareMatched(left: MatchedManagedPullRequest, right: MatchedManagedPullRequest): number {
    return comparePullRequests(left.existing, right.existing);
}
