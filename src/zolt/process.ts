...
export async function createZoltEnvironment(
    source: NodeJS.ProcessEnv,
    selectedNames: readonly string[],
    githubToken: string,
    registerSecret: (value: string) => void,
): Promise<ZoltEnvironment> {
    ...
    const environment = Object.fromEntries(
        Object.entries(minimalZoltEnvironment(source))
...
export function selectedCredentialValues(
    source: NodeJS.ProcessEnv,
    selectedNames: readonly string[],
): readonly string[] {
...
export function minimalZoltEnvironment(source: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
    ...
    if (source.SystemRoot !== undefined) result.SystemRoot = source.SystemRoot;
    return result;
}
...
export async function runZolt(
    ...
    timeout = ZOLT_COMMAND_TIMEOUT_MS,
    ...
    return new Promise((resolvePromise, reject) => {
        ...
        function finish(error?: Error, result?: ProcessResult): void {
            ...
            else resolvePromise(result as ProcessResult);
        }
    });
}
