...
async function removeTransientMutationLocks(workspace, target) {
    const paths = [
        '.zolt/workspace-mutation.lock',
        `.zolt/lockfile-mutations/${target.zoltLockfilePath}.lock`,
    ];
    for (const path of paths)
        await removeTransientMutationLock(workspace, target.zoltRoot, path);
}
async function removeTransientMutationLock(workspace, zoltRoot, transientPath) {
...
function validateChanges(changes, result, target) {
    if (changes.deleted.length !== 0
        ...
        throw actionError('ZOLT-EXECUTE-003', `Zolt exact update deleted entries or changed file modes: deleted [${changes.deleted.join(', ')}], modes [${changes.modeChanged.join(', ')}], missing directories [${changes.missingDirectories.join(', ')}].`);
    }
    if (changes.added.some((path) => path !== target.lockfilePath)) {
        throw actionError('ZOLT-EXECUTE-003', `Zolt exact update added a file outside its manifest/root-lock boundary: ${changes.added.join(', ')}.`);
    }
    const allowed = new Set([target.manifestPath, target.lockfilePath]);
    if (changes.paths.some((path) => !allowed.has(path))) {
        throw actionError('ZOLT-EXECUTE-003', `Zolt exact update changed a file outside its manifest/root-lock boundary: ${changes.paths.join(', ')}.`);
    }
