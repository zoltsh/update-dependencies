...
async function discoverWorkspace(repository) {
    ...
    while (contained(repository.workspace, current)) {
        ...
        if (legacy !== undefined && modern) {
            throw new UpdateDependenciesError('ZOLT-WORKSPACE-007', `Workspace at ${repositoryRelativeRoot(repository.workspace, current)} declares both zolt-workspace.toml and [workspace] in zolt.toml.`);
        }
        if (legacy !== undefined)
            return { manifest: legacy, root: current };
        if (modern && rootConfig !== undefined)
            return { manifest: rootConfig, root: current };
        if (current === repository.workspace)
            break;
        current = dirname(current);
    }
    return undefined;
}
async function workspace(repository, location) {
...
async function standalone(repository, root) {
...
async function requiredFile(candidate, workspaceRoot, label) {
    const file = await regularFileInside(candidate, workspaceRoot);
    if (file === undefined)
        throw new UpdateDependenciesError('ZOLT-WORKSPACE-002', `Required ${label} was not found.`);
    return file;
}
async function regularFileInside(candidate, workspaceRoot) {
    try {
...
function repositoryRelativeRoot(workspaceRoot, candidate) {
    const value = relative(workspaceRoot, candidate);
