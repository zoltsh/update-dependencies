...
export function registeredSecrets(environment, extras = []) {
...
export function publicText(value, secrets) {
...
export function publicErrorMessage(error, secrets) {
    ...
    if (error instanceof Error)
        return `ZOLT-UNEXPECTED-001: ${publicText(error.message, secrets)}`;
    return `ZOLT-UNEXPECTED-001: ${publicText(String(error), secrets)}`;
}
function jsonSecretVariants(secrets) {
