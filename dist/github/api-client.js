...
export function createGitHubRequester(token, dependencies = {}) {
    requireToken(token);
    ...
    return async (request) => {
        ...
        try {
            return Object.freeze({ status: response.status, value: JSON.parse(source) });
        }
        catch (error) {
...
function encodeRequestBody(value) {
    try {
...
function canonicalApiUrl(value) {
    let parsed;
    try {
...
function githubApiError(message, cause) {
