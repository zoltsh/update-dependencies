...
import { MAX_ARCHIVE_DECOMPRESSION_RATIO, MAX_ARCHIVE_ENTRIES, MAX_ARCHIVE_ENTRY_BYTES, MAX_EXTRACTED_BYTES, } from '../constants.js';
import { actionError } from '../errors.js';
export async function inspectArchive(archive, expectedRoot) {
...
export async function extractArchive(archive, destination, expectedRoot) {
    const compressed = (await stat(archive)).size;
    const tar = await gunzipBounded(archive);
    if (compressed === 0 || tar.length / compressed > MAX_ARCHIVE_DECOMPRESSION_RATIO) {
        throw archiveError('The Zolt archive exceeds the decompression ratio limit.');
...
async function gunzipBounded(archive) {
    ...
    try {
        ...
        if (error instanceof Error && error.name === 'UpdateDependenciesError')
            throw error;
        throw archiveError('Could not decompress the pinned Zolt archive.', error);
    }
    return Buffer.concat(chunks, total);
}
function parseTar(tar, expectedRoot) {
    const entries = [];
    const paths = new Set();
    let offset = 0;
    ...
    while (offset + 512 <= tar.length) {
        ...
        if (type === undefined)
            throw archiveError(`Archive entry ${path} has a forbidden type.`);
        ...
        if ((mode & 0o7000) !== 0)
            throw archiveError(`Archive entry ${path} contains privilege bits.`);
        const key = path.replace(/\/$/u, '').toLowerCase();
        if (paths.has(key))
            throw archiveError(`Archive contains a duplicate or case-colliding path: ${path}.`);
        paths.add(key);
        const end = offset + size;
        if (end > tar.length)
            throw archiveError('The Zolt archive is truncated.');
        ...
        if (path === expectedBinary) {
            if (type !== 'file' || foundBinary)
                throw archiveError('Archive contains an invalid Zolt executable.');
            foundBinary = true;
        }
        entries.push({ content, mode, path: path.replace(/\/$/u, ''), type });
        if (entries.length > MAX_ARCHIVE_ENTRIES)
            throw archiveError('The Zolt archive has too many entries.');
        offset += Math.ceil(size / 512) * 512;
    }
    if (!ended || !foundBinary)
        throw archiveError(`Archive is missing ${expectedRoot}/bin/zolt or its end marker.`);
    for (let index = offset; index < tar.length; index += 1) {
        if (tar[index] !== 0)
            throw archiveError('The Zolt archive contains trailing data.');
    }
    return entries;
}
function validateChecksum(header) {
    ...
    if (actual !== expected)
        throw archiveError('The Zolt archive contains an invalid tar checksum.');
}
function validatePath(path, expectedRoot) {
...
function octal(field, label) {
    ...
    if (!/^[0-7]+$/u.test(value))
        throw archiveError(`The Zolt archive has an invalid ${label} field.`);
