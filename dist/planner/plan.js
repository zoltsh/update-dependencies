...
export function planUpdates(report, selection, inputs) {
    const eligible = [];
    const blocked = [];
    const outsidePolicy = [];
    const seen = new Set();
...
function selectTarget(entry, ceiling) {
    ...
    if (ceiling === 'minor')
        return paired(entry.selectedInMajor, entry.selectedInMajorClass, 'selectedInMajor');
    return paired(entry.selectedLatest, entry.selectedLatestClass, 'selectedLatest');
}
function paired(version, changeClass, label) {
