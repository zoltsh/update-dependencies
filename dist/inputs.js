...
export function readInputs(core, registerSecret) {
    const githubToken = core.getInput('github-token');
    if (githubToken === '')
        throw actionError('ZOLT-INPUT-010', 'github-token must not be empty.');
    ...
    return {
        ...
        githubToken,
        includePrereleases: booleanInput(core.getInput('include-prereleases') || 'false', 'include-prereleases'),
        openPullRequestsLimit: integerInput(core.getInput('open-pull-requests-limit') || '5', 'open-pull-requests-limit', 0, 100),
        registryEnv: environmentNames(core.getInput('registry-env')),
        selectors: lines(core.getInput('selectors'), 'selectors', 100),
        updateCeiling: choice(core.getInput('update-ceiling') || 'minor', 'update-ceiling', ['patch', 'minor', 'major']),
        workspace: choice(core.getInput('workspace') || 'auto', 'workspace', ['auto', 'true', 'false']),
    };
}
...
function integerInput(value, name, minimum, maximum) {
...
function lines(value, name, maximum) {
    const result = [...new Set(value.split(/\r?\n/u).map((entry) => entry.trim()).filter(Boolean))];
    if (result.length > maximum)
        throw actionError('ZOLT-INPUT-004', `${name} accepts at most ${maximum.toString()} entries.`);
...
function nonEmpty(value, name) {
    if (value.trim() === '')
        throw actionError('ZOLT-INPUT-008', `${name} must not be empty.`);
    if (/[\u0000-\u001F\u007F]/u.test(value))
        throw actionError('ZOLT-INPUT-009', `${name} contains control characters.`);
    return value;
}
